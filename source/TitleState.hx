...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('shun_icon'));
		add(ngSpr);
		ngSpr.visible = false;
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				case 1:
					#if PSYCH_WATERMARKS
					createCoolText(['Psych Engine made by:'], 15);
					#else
					createCoolText(['Psych Engine made by:'], 15);
					#end
				...
				case 5:
					#if PSYCH_WATERMARKS
					createCoolText(['VS Shun directed by:'], -40);
					#else
					createCoolText(['VS Shun directed by:'], -40);
					#end
				case 7:
					addMoreText('Shuntex', 15);
					ngSpr.visible = true;
				// credTextShit.text += '\nNewgrounds';
				case 8:
					deleteCoolText();
