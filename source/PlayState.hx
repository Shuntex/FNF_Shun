...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Mediocre', 0.69], //From 60% to 68%
		['Decent', 0.7], //69%
		['Acceptable', 0.8], //From 70% to 79%
	...
	var chromaticShader:ChromaticAberrationEffect = new ChromaticAberrationEffect(0.006);
	...
	override public function create()
	{
		...
		switch(curStage)
		{
			...
			case 'temple2':
				var shunTrail = new FlxTrail(dad, null, 4, 24, 0.1, 0.001); //nice
				insert(members.indexOf(dadGroup) - 1, shunTrail);
			case 'temple3':
				var shunTrail = new FlxTrail(dad, null, 4, 24, 0.1, 0.001); //nice
				insert(members.indexOf(dadGroup) - 1, shunTrail);
		}
		...
		DiscordClient.changePresence("V.S. SHUN" + detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
		#end
  ...
  public function clearShaderFromCamera(cam:String){
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence("V.S. SHUN" + detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, songLength);
		#end
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer.finished)
			{
				DiscordClient.changePresence("V.S. SHUN" + detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
			}
			else
			{
				DiscordClient.changePresence("V.S. SHUN" + detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Game Over - " + "FNF V.S. SHUNTEX", SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
	...
	{
		if (curSong == 'Retribution' || curSong == 'Fallen' || curSong == 'Fallen-Rage')
		{
			//camGame.setFilters([new openfl.filters.ShaderFilter(chromaticShader.shader)]);
			//camOther.setFilters([new openfl.filters.ShaderFilter(chromaticShader.shader)]);
			camHUD.setFilters([new openfl.filters.ShaderFilter(chromaticShader.shader)]);
